...
def _hits(
    passwords: list[str], source_url: str, method: str, location: str
) -> list[PasswordHit]:
    result = [
        PasswordHit(
            password=p,
...
def scan_raw_bytes(file_path: str, source_url: str) -> list[PasswordHit]:
    ...
    return _hits(
        find_passwords(text_latin1), source_url, "pixel_scan", "raw bytes latin-1"
    )
...
def scan_pil_info(file_path: str, source_url: str) -> list[PasswordHit]:
    ...
    with Image.open(file_path) as img:
        return _hits(
            find_passwords(str(img.info)), source_url, "pixel_scan", "pil image.info"
        )
...
def scan_pixel_dumps(file_path: str, source_url: str) -> list[PasswordHit]:
    ...
    with Image.open(file_path) as img:
        ...
        hits.extend(
            _hits(
                find_passwords(hex_row), source_url, "pixel_scan", "row-order hex dump"
            )
        )
        ...
        hits.extend(
            _hits(
                find_passwords(ascii_row),
                source_url,
                "pixel_scan",
                "row-order ascii dump",
            )
        )

        pixels_col = [
            rgb_img.getpixel((x, y)) for x in range(width) for y in range(height)
        ]

        hex_col = "".join(f"{r:02x}{g:02x}{b:02x}" for r, g, b in pixels_col)
        hits.extend(
            _hits(
                find_passwords(hex_col),
                source_url,
                "pixel_scan",
                "column-order hex dump",
            )
        )
        ...
        hits.extend(
            _hits(
                find_passwords(ascii_col),
                source_url,
                "pixel_scan",
                "column-order ascii dump",
            )
        )
...
def scan_lsb_stego(file_path: str, source_url: str) -> list[PasswordHit]:
    ...
    return _hits(
        find_passwords(lsb_text), source_url, "pixel_scan", "lsb steganography"
    )

...
def _hits(passwords: list[str], source_url: str, method: str, location: str, payload_type: AssetType) -> list[PasswordHit]:
    hits = [
        PasswordHit(password=p, source_url=source_url, method=method, location=location, payload_type=payload_type)
        for p in passwords
    ]
    for hit in hits:
...
def scan_raw_source(response: httpx.Response, payload_type: AssetType) -> list[PasswordHit]:
    ...
    logger.debug("scan_raw_source: %d bytes scanned for %s", len(response.content), source_url)
    return _hits(passwords, source_url, "raw_source", "raw_source", payload_type)


def scan_visible_text(soup: BeautifulSoup, source_url: str, payload_type: AssetType) -> list[PasswordHit]:
    passwords = find_passwords(soup.get_text())
    return _hits(passwords, source_url, "visible_text", "visible_text", payload_type)


def scan_headers_and_cookies(response: httpx.Response, payload_type: AssetType) -> list[PasswordHit]:
    source_url = str(response.url)
    hits: list[PasswordHit] = []

    for header_name, header_value in response.headers.items():
        passwords = find_passwords(header_value)
        hits.extend(_hits(passwords, source_url, "header", f"header:{header_name}", payload_type))

    for cookie_name, cookie_value in response.cookies.items():
        passwords = find_passwords(f"{cookie_name}={cookie_value}")
        hits.extend(_hits(passwords, source_url, "cookie", f"cookie:{cookie_name}", payload_type))

    logger.debug("scan_headers_and_cookies: scanned %d headers, %d cookies for %s", len(response.headers), len(response.cookies), source_url)
    return hits
